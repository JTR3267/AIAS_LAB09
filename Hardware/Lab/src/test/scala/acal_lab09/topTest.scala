...
class topTest(dut:top) extends PeekPokeTester(dut){
    ...
    while(!peek(dut.io.Hcf)){
        ...
        var Flush_WB_ID_DH = peek(dut.io.Flush_WB_ID_DH).toInt
        ...
        println(s"[Flush_WB_ID_DH ] ${"%1d".format(Flush_WB_ID_DH)} [Flush_BH ] ${"%1d".format(Flush_BH)} [Stall_WB_ID_DH ] ${"%1d".format(Stall_WB_ID_DH)} [Stall_MA ] ${"%1d".format(Stall_MA)} ")
        println("==============================================")
